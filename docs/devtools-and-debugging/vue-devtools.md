...
The library auto-wires the inspector + timeline when the dep is present at runtime:
...
If the peer dep isn't installed at runtime, nothing breaks — the inspector simply doesn't register; the form library works as usual. Treating the integration as optional means a freshly-cloned project doesn't fail to start just because `@vue/devtools-api` isn't on disk.
