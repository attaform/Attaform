...
description: Opt in per form and per field, pick a storage backend, and Attaform saves drafts across reloads with schema-aware hydration, sensitive-name guards, and zero ceremony beyond the persist option.
metaRows:
  - label: Category
    value: Module
...
The resolved list is one configurable source of truth for "what counts as sensitive." (DevTools renders raw values by design; the list does not gate display.)
