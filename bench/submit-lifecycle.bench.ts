 ...
 * backend, write the returned errors. This bench times the round-trip
 * of those steps on a moderate form so regressions in
 * `process-form.ts` / the error-store writers surface in CI without
...
describe('submit-lifecycle: validate → handleSubmit → setErrors', () => {
  ...
  const handler = form.handleSubmit(
    ...
    async (_values) => {
      // Simulate a 422: the server's ValidationError[] handed straight
      // to setErrors, no parse step.
      form.setErrors([
        { path: ['email'], message: 'already taken', code: 'api:duplicate' },
        { path: ['profile', 'age'], message: 'must be 18+', code: 'api:too-young' },
      ])
    }
  )
