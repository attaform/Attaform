...
import { useForm, useWizard } from 'attaform/zod-v4'
...
import { shapeFor, zodV4SchemaFor } from '../../shared/scenarios'
import ArrayRow from '../attaform/ArrayRow.vue'
import Field from '../attaform/Field.vue'
import type { AttaformForm } from '../attaform/types'
...
const unsupported = (op: string): never => {
  throw new Error(`bench: attaform-zod4 adapter does not drive "${op}" in this scenario`)
}
...
export const attaformZod4Adapter: BenchAdapter = {
  ...
  async mount(container, opts): Promise<MountHandle> {
    ...
    const schema = zodV4SchemaFor(opts.scenario, opts.params)
    const validateOn: 'change' | 'blur' = opts.trigger === 'blur' ? 'blur' : 'change'
    mountSeq += 1
    const key = `bench-attaform-zod4-${opts.scenario}-${opts.seed}-${mountSeq}`
