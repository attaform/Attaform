...
<template>
  <div class="demo layout split">
    <form class="card" :aria-busy="form.hydrating" @submit.prevent>
      <header>
        <h4>Draft loader</h4>
        <span v-if="form.hydrating" class="badge busy">loading draft…</span>
        <span v-else-if="form.hydrateError" class="badge error">load failed</span>
        <span v-else class="badge success">ready</span>
      </header>
      ...
      <div v-if="form.hydrateError" class="banner failed" role="alert">
        {{ form.hydrateError.message }}. Click <strong>Rehydrate</strong> to retry.
      </div>

      <div class="actions">
        <button type="button" class="primary" :disabled="form.hydrating" @click="onRehydrate">
