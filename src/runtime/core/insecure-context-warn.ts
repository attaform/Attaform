 ...
 * unique key emits at most one warning per app-load — the built-in
 * persistence storage adapters use this to surface "this feature
 * noop'd because you're on plain HTTP" without spamming the console
 * once per form mount.
 *
 * The gate matches `window.isSecureContext`, which is `true` for HTTPS
 * in production AND localhost in development. Plain HTTP on a real
 * hostname noops with this warning.
 */
export type InsecureContextFeature = 'persist:local' | 'persist:session'
 ...
 * `window` is undefined) returns `false` — built-in persistence is
