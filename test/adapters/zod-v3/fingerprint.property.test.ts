...
/**
 * Statistical injectivity test for the zod-v3 schema fingerprint.
 * Parallel coverage to the v4 property suite, adapted to v3's
 * `_def.typeName`-based introspection surface.
 *
...
function buildZod(recipe: Recipe): z.ZodTypeAny {
  switch (recipe.kind) {
    case 'string': {
      let s = z.string()
    ...
    case 'default-literal':
      return (buildZod(recipe.inner) as z.ZodTypeAny).default(recipe.value as never)
    case 'catch-literal':
      return (buildZod(recipe.inner) as z.ZodTypeAny).catch(recipe.value as never)
    case 'array':
    ...
    case 'tuple': {
      ...
      return z.tuple(
        items as unknown as [z.ZodTypeAny, ...z.ZodTypeAny[]]
    ...
    case 'union': {
      ...
      return z.union(opts as unknown as readonly [z.ZodTypeAny, z.ZodTypeAny, ...z.ZodTypeAny[]])
    }
    case 'dunion': {
      ...
      return z.discriminatedUnion(
        ...
        opts as unknown as readonly [z.AnyZodObject, ...z.AnyZodObject[]]
      )
    }
    case 'record':
...
describe('v3 fingerprint — statistical injectivity (property-based)', () => {
