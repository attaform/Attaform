...
- **Every library runs in its fastest idiomatic configuration.** Debounces are neutralized, validation triggers are normalized, and array and union primitives use each library's native fast path. Attaform is measured on its shipping configuration, never a relaxed setting.
...
Attaform is the heaviest in the cohort, and the heaviest in its own layer. That is the cost of shipping reactive form state, deep schema binding, and the whole display-state model in one zero-dependency package, and it is the honest price of admission. The figure is what a code-split build executes before the form is interactive; opt-in layers like the `v-register` directive, undo and redo, and the multi-step wizard live behind separate imports this minimal form never pays for.

The table groups by layer for the reason the fairness rule above gives: a validation-only engine is not a cheaper version of a form-state library, so the two should not be read as a single ranking. Each row also carries the one line the arena can derive about what that library is for, from its adapter's own declared design point. And the Zod rows share a floor, quoted under the table: building nothing but the two-field schema and the parse call every Zod adapter makes already costs most of a small library, so that much of each of those figures is identical bytes before any form library. That figure is measured by the same build as the rows rather than written here, so it cannot drift away from the chart it explains.
...
- **Bundle is the initial load, with any deferred part beside it.** The build code-splits the way a real bundler does. A row's figure is the entry chunk plus every chunk it reaches through static imports, minified and gzipped, with the validator weighed in. Anything reached only through a dynamic import is weighed separately and shown beside the figure as deferred, so a row with no deferred badge loads in one chunk. Vue is external, since every app ships it once.
- **The shared-validator floor is a floor, not a subtraction.** The figure quoted under the bundle table is the schema and parse call every Zod row has in common. An adapter that reaches further into Zod (async parsing, issue introspection) pulls a little more than that; none pulls less. So it is the right figure for "how much of this is not the form library at all" and the wrong one for backing out an exact library-only size per row.
