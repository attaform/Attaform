/**
 * The form store's DOM slice, kept out of the kernel so the kernel never
 * pays for it.
 *
 * Element registration, no-latch host anchors, the DOM-order sort cache,
 * focus/blur listeners and first-error focus resolution all live in this
 * module, inside the directive cluster's lazy graph. The kernel keeps a
 * `domBinding` slot (a shallow ref, `null` until armed) plus the narrow
 * field-record transitions this module drives (`noteDomConnected` /
 * `noteDomDisconnected`). Every eager reader (`field.element` /
 * `field.elements`, invalid-submit focus, `focusFirstError` /
 * `scrollToFirstError`) reads through the slot and treats `null` as
 * "nothing registered", which is the truth: with no directive and no
 * `useRegister` in the app, nothing could have registered an element.
 *
 * Arming is explicit dependency injection. The directive and `useRegister`
 * pass `createDomBinding` into the RegisterValue's internal
 * `ensureDomBinding` before any element call (see `armDomBinding`). An
 * import-time module-slot side effect would be tree-shaken under the
 * package's `"sideEffects": false`, and per-call injection also keeps
 * duplicate-package-copy apps coherent, since whichever copy's cluster
 * runs arms the store it is bound to.
 */
...
// element-property key for stashed focus/blur handlers. Same reasoning as
// `assignKey` in core/assigner-pipeline.ts.
...
// thumb, a listbox or combobox trigger `<button>`, a real `<input>` segment.
...
// BubbleInput and the like) are excluded, never being the user's focus
...
/**
 * The focus-first-error target for a no-latch `v-register` component host.
 * The host binds a value but owns no single control, so resolve its root to
 * the first visible, enabled focusable descendant: the tab stop a keyboard
 * user entering the widget lands on, whether that is the first radio of a
 * group, a slider thumb, a listbox trigger or the first pin segment. Falls
 * back to the host root when nothing focusable is found, so
 * `scrollToFirstError` still has a target even where `focus()` cannot land.
 * `offsetParent === null` mirrors `getFirstErrorElement`'s own visibility
 * gate, skipping `display:none` subtrees.
 */
...
function attachFocusListeners(
  ...
  // lifecycle runs and the listeners above attach, and a programmatic
  // `.focus()` from a parent's `onMounted` has the same race. Either way
  // the focus event has come and gone by wire-up time and the handler never
  // runs. Probing `document.activeElement` once immediately after attaching
...
export function createDomBinding(kernel: DomBindingKernel): AttaformDomBinding {
  ...
  // Per-element form-instance tag, in a WeakMap so a detached element GCs
  // with no bookkeeping. `getFirstErrorElement` reads it to scope focus and
  // scroll targets to the registering `useForm()` instance, which matters
  // when two instances share a `key` and both register into one `elements`
  // Map.
  const elementToFormInstance = new WeakMap<HTMLElement, string>()

  // No-latch component-host focus anchors. A host binding value through the
  // v-model channel but latching no inner control enters no `elements`
  // record and would be invisible to `getFirstErrorElement`. Keyed by path,
  // the anchor resolves to a focusable descendant at submit time, and
  // `formInstanceId` scopes it exactly as a registered control is scoped. A
  // path never holds both an anchor and a registered element: `attach`
  // drops the anchor when a control latches.
  ...
  function attach(
    ...
    // Form-element semantics (registration plus focus listeners) are gated
    // behind the interactive tag set, so a component wrapper div cannot
    // register by accident when fallthrough attributes carry the directive
    // past the intended `<input>` / `<select>` / `<textarea>`.
    if (!INTERACTIVE_TAG_NAMES.has(element.tagName)) return
    const { key } = canonicalizePath(segments)
    const record = elements.get(key)
    // `markRaw` keeps the HTMLElement out of Vue's auto-proxy machinery:
    // DOM nodes carry circular refs and external state that fight
    // reactivity, and a consumer comparing `===` against the original ref
    // expects back what they registered. The Set itself stays reactive, so
    // an add or delete on an existing record fires FieldState's `element`
    // and `elements` accessors.
  ...
  function markHostConnected(
    ...
    // directive calls this from its mount and unmount on the client, so it
    // is the authoritative connect/disconnect for a no-latch host: there is
    // no element-Set entry to carry `connected` for one.
    const { key } = canonicalizePath(segments)
    if (connected) {
  ...
  function getFirstErrorElement(
    ...
    for (const entry of sortedRegistrationsCache) {
      // Scope to this form instance. When two `useForm()` calls share a key
      // both write into `elements` and `hostTargets`, and this filter keeps
      ...
      // `el.isConnected` covers the "component unmounted, element removed
      // from DOM" cases that lag the FieldRecord.connected flag.
      // `el.offsetParent === null` catches `display:none` and its ancestor
      // chain; the browser will not focus or scroll to a hidden element
      // anyway, so keep walking. For a host entry the element is the host
      // root, and a hidden host is skipped the same way.
      if (!entry.element.isConnected) continue
      if (entry.element.offsetParent === null) continue

      // Route through the canonical merged read so focus and scroll target
      // exactly the paths that surface an error in `form.errors` and
      // `field.errors`. The blank-required class lives only in
      // `derivedBlankErrors`, never in `schemaErrors`, so consulting the
      // schema and user stores alone silently skips the first empty
      // required field, the very class this policy exists to jump to
      // (#468). `getErrorsForPath` folds in all three channels, which is
      // what keeps the focus target and the visible error set together.
      ...
      // The resolver falls back to the root itself, so scroll-to-error keeps
