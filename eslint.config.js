...
export default [
  ...
  // `meta.vue`, `checkbox.vue`), where the slug IS the reader-facing surface
  ...
  // Site Vue/TS files. No `project` here, since the site has its own
  ...
  // File/folder ignores (Flat Config doesn't respect .eslintignore by default)
  {
    ignores: [
      ...
      // shaped trap as the _pagefind bundles below: flat config does
      // not read .gitignore, and linting these dominates the whole-repo
      // lint runtime.
      'apps/site/.repl-cache/**',
      // Pagefind search bundles: minified `.js` regenerated by the site
      // dev/build. Gitignored, but eslint flat config does not read
      // .gitignore, and 4000+ "violations" in minified code overflow the
      // stylish formatter (RangeError: Invalid string length). Mirror the
