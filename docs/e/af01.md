...
condition: 'The schema and the adapter disagree on the Zod major version'
---
...
- Import from `attaform/zod`. It routes each schema to the adapter that can read it, so a mismatch stops being possible, and under the `attaform/vite` plugin the specifier is resolved at build time so only one adapter ships.
