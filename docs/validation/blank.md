...
description: A storage / display side-channel that handles numeric inputs cleanly — storage holds 0 while the user sees empty. blankPaths tracks the divergence so submit doesn't lie.
metaRows:
  - label: Category
    value: Field-state
  - label: Auto-marks
  ...
  - label: Manual opt-in
    value: unset sentinel — any primitive
    kind: code
  - label: Error code
...
> A storage / display side-channel that handles cleared numeric inputs cleanly — storage holds `0`, the user sees empty, and the form doesn't silently submit `0` for an unfilled required field.
...
`unset` works at any primitive leaf and adds the path to `blankPaths`. Combined with required schemas, it surfaces a `atta:no-value-supplied` error reactively — same lifecycle as the numeric case, just driven by your intent rather than runtime inference.
...
- [`unset` — the absent sentinel](/docs/writing-and-mutating/unset) — the imperative way to opt any primitive into blank.
