...
- **Function slot**: `(ctx) => Form | string | null | undefined`. Re-evaluates reactively as `ctx.forms.<key>.values` mutate. The picked result replaces the slot's compiled step. Returning either nullish value drops the slot.
...
[`gate()`](/docs/multistep/gate) joins them as a wrapper rather than a fifth kind: it takes any of the four, marks that position a hard prerequisite, and leaves how the slot resolves alone. A function slot can return one, so whether a position is a gate at all can be a runtime decision.
...
| `defaultStatuses` | `Record<key, FormStatusSeed>` &#124; sync factory &#124; async factory | unset               | Seed payload used while a form's defaults are still resolving, and the only way to start a `gate()` cleared. See [Statuses](/docs/multistep/statuses).              |
...
| `focusFirstError` | `boolean`                                                              | `true`              | On submission failure, jump to the first failing form and run its `applyInvalidSubmitPolicy()` (honoring that form's `focusOnInvalidSubmit` choice).                |
...
| `activeForm`         | A live view of the active step's form; operating through it always targets the current step. It is a view, not the handle itself, so reach for `wizard.forms[currentStep]` when you need the raw form. Narrows in lockstep with `currentStep`. |
...
| `submitError`        | The `Error` a submit callback threw, parked rather than re-thrown. `null` otherwise. See [handleSubmit](/docs/multistep/handle-submit#when-a-callback-throws).                                                                                 |
| `submissionAttempts` | Count of `wizard.handleSubmit` runs. Bumped before the callbacks, so a read inside `onSubmit` / `onError` already counts the run in progress. See [Aggregates](/docs/multistep/aggregates#wizardsubmissionattempts-vs-per-form-attempts).      |
...
| `reset`              | Zeros wizard lifecycle, resets every form, returns to `steps[0]` and persists that key, flips `done` back, re-seals every gate, and re-applies the `defaultStatuses` gate seed.                                                                |
...
`next` / `back` / `goTo` are positional navigation, and they do not validate. To gate an advance on the active step, use `wizard.tryNext()` (or compose `wizard.activeForm.handleSubmit(() => wizard.next())` when you need custom valid or invalid handling). Out-of-bounds calls dev-warn and no-op; mid-submission navigation is blocked until `wizard.submitting` clears. A wizard wired into a checkout or signup never throws on navigation.

One carve-out, and it is the whole point of [`gate()`](/docs/multistep/gate): `next()` on an uncleared gate step behaves like `tryNext()`, submitting the step and moving only on a clean pass. So wiring a Next button straight to `next()` can never step around a prerequisite, and `goTo` refuses a target sealed behind one. Once the gate clears, `next()` is plain navigation again.
...
- **`restore` returns a key not in the compiled steps.** Dev-warn, and what happens next depends on when it arrives. On the first read, at construction, the wizard falls back to the first step. On any later read, when `restore` re-fires because the URL changed, the unknown key is ignored and the pin stays where it is, so a hand-edited `?step=` never yanks a user out of the step they were on.
