...
/**
 * v3 constraint-merge parity (D19). Both adapters fold
 * `config.constraints` into schema defaults through core's `mergeDeep`:
 * an array replaces wholesale, and an explicit `null` or `undefined`
 * override is honoured. An element-wise merge instead diverges on the
 * audit's canonical repro:
 *
 *   schema: z.object({ tags: z.array(z.string()).default(['x', 'y']) })
 *   constraints: { tags: ['a'] }
 *   wholesale:    { tags: ['a'] }
 *   element-wise: { tags: ['a', 'y'] }
 *
 * The v4 half is `test/adapters/zod-v4/constraint-merge-parity.test.ts`,
 * and dual-green is the parity proof.
 */
