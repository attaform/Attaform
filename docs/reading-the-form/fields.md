...
metaRows:
  ...
  - label: Read shape per leaf
    value: FieldState (30 properties)
    kind: code
---

# `fields`

> A reactive Proxy keyed by schema paths. Every leaf surfaces a 30-property FieldState: state bits, value reads, validation reads, DOM handles, and schema metadata, all in one snapshot the form keeps in sync as users interact.
...
Each leaf exposes a 30-property `FieldState` object. The properties fall into five jobs:
